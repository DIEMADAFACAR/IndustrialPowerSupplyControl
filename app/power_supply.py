...
file_handler = logging.FileHandler('power_supply.log')
...
class PowerSupply:
    ...
    DISABLE_OUTPUT_COMMAND_TEMPLATE = ":OUTPut{channel}:STATe 0\n"
    VOLTAGE_QUERY_COMMAND_TEMPLATE = ":MEASure{channel}:VOLTage?\n"
    CURRENT_QUERY_COMMAND_TEMPLATE = ":MEASure{channel}:CURRent?\n"
    ...
    async def connect(self):
        try:
            ...
            logger.error(f"Failed to connect to the power supply: {e}")
    ...
    async def disconnect(self):
        try:
            ...
            logger.error(f"Error while disconnecting from the power supply: {e}")
    ...
    async def send_command(self, command):
        try:
            ...
            logger.error(f"Error while sending command to the power supply: {e}")
    ...
    async def receive_response(self):
        try:
            ...
            logger.error(f"Error while receiving response from the power supply: {e}")

    async def set_channel_current(self, channel, current):
        try:
            await self.send_command(f":MEASure{channel}:CURRent {current}\n")
        except Exception as e:
            logger.error(f"Error while setting current for channel {channel}: {e}")
    ...
    async def set_channel_voltage(self, channel, voltage):
        try:
            ...
            logger.error(f"Error while setting voltage for channel {channel}: {e}")
    ...
    async def query_voltage(self, channel):
        try:
            ...
            logger.error(f"Error while querying voltage for channel {channel}: {e}")
    ...
    async def query_all_channel_status(self):
        try:
            ...
            logger.error(f"Error while querying all channel status: {e}")
    ...
    async def poll_telemetry(self, interval=10):
        while True:
            try:
                ...
                logger.error(f"Telemetry polling error: {str(e)}")
...
if __name__ == "__main__":
    ...
    async def main():
        power_supply = PowerSupply(host="your_host", port="")
        await power_supply.connect()

        # Установить ток для канала 1
        await power_supply.set_channel_current(1, 5)

        # Запросить состояние всех каналов
